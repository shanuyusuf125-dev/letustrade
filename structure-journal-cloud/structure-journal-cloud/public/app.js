...
let activeTf = null;
let currentAccountFilter = ''; // '' = all accounts
...
function tradesUrl() {
  return currentAccountFilter ? `/api/trades?account=${encodeURIComponent(currentAccountFilter)}` : '/api/trades';
}

// Only show the account dropdown at all if there's more than one account
// in the data - a single-account setup (the common case) shouldn't have
// to look at a filter that never does anything.
async function loadAccounts() {
  try {
    const res = await fetch('/api/accounts');
    const accounts = await res.json();
    if (accounts.length <= 1) {
      accountFilterEl.style.display = 'none';
      return;
    }
    accountFilterEl.innerHTML = '<option value="">All accounts</option>' +
      accounts.map(a => `<option value="${a}">${a === 'unknown' ? 'Unknown (pre-account-id trades)' : a}</option>`).join('');
    accountFilterEl.style.display = 'block';
  } catch (e) { /* fine, just don't show the filter */ }
}

accountFilterEl.addEventListener('change', () => {
  currentAccountFilter = accountFilterEl.value;
  loadTrades();
