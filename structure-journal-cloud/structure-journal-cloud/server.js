...
// table that already existed from before account_id existed - so check
// and ALTER it in ourselves if it's missing.
...
app.post('/api/sync', (req, res) => {
  ...
  const upsertTrade = db.prepare(`
    INSERT INTO trades (position_id, symbol, direction, open_price, open_time, open_volume, open_sl, open_tp,
                         close_price, close_time, close_profit, close_sl, close_tp, status, account_id)
...
// ---------------------------------------------------------------------
// Full backup export - same shape as the local app's /api/export, built
// from the cloud DB instead of local files. Useful precisely because
// journal.db here gets wiped on every Render redeploy - download this
// before you redeploy if you want a copy of whatever's only made it to
// the cloud (e.g. notes/ratings you added from your phone).
// ---------------------------------------------------------------------
app.get('/api/export', (req, res) => {
  const trades = getTradesList();
  const snapRows = db.prepare('SELECT * FROM snapshots').all();
  const snapshots = {};
  for (const r of snapRows) {
    snapshots[r.position_id] = { structureRaw: r.structure_raw, candles: JSON.parse(r.candles_json || '{}') };
  }
  const strategies = db.prepare('SELECT name FROM strategies').all().map(r => r.name);
  const goalRows = db.prepare('SELECT period_type, target FROM goals').all();
  const goals = { day: 0, week: 0, month: 0 };
  for (const r of goalRows) goals[r.period_type] = r.target;

  res.setHeader('Content-Disposition', `attachment; filename="journal-backup-${new Date().toISOString().slice(0, 10)}.json"`);
  res.json({ exportedAt: new Date().toISOString(), trades, snapshots, strategies, goals });
});
...
function getTradesList(accountFilter) {
  ...
  if (accountFilter) {
    if (accountFilter === 'unknown') {
      trades = trades.filter(t => !t.account_id);
    } else {
      trades = trades.filter(t => String(t.account_id) === String(accountFilter));
    }
  }
  trades.sort((a, b) => {
...
app.get('/api/trades', (req, res) => res.json(getTradesList(req.query.account)));
...
app.get('/api/accounts', (req, res) => {
  ...
  const accounts = Array.from(ids).sort();
  if (hasUnknown) accounts.push('unknown');
  res.json(accounts);
});
